...
WINDOW_BACKBONES: Dict[str, Dict[str, Any]] = {
    "delayed_post": {
        "baseline": (-1.0, -0.1),
        "delayed": (0.2, 0.7),
        "stim": (0.2, 0.7),
        "post": (0.7, 1.4),
        "window_type": "delayed_minus_baseline",
        "description": "Delayed/post backbone used in the maintained branch.",
    },
    "stim_post": {
        "baseline": (-1.0, -0.1),
        "delayed": (0.0, 1.0),
        "stim": (0.0, 1.0),
        "post": (1.0, 2.0),
        "window_type": "stim_minus_baseline",
        "description": "Stimulation-aligned backbone imported from the finance branch.",
    },
}
DEFAULT_WINDOW_BACKBONE = "delayed_post"
DEFAULT_WINDOWS = {
    "baseline": WINDOW_BACKBONES[DEFAULT_WINDOW_BACKBONE]["baseline"],
    "delayed": WINDOW_BACKBONES[DEFAULT_WINDOW_BACKBONE]["delayed"],
    "post": WINDOW_BACKBONES[DEFAULT_WINDOW_BACKBONE]["post"],
}
WINDOWS = {
    "baseline": WINDOW_BACKBONES[DEFAULT_WINDOW_BACKBONE]["baseline"],
    "delayed": WINDOW_BACKBONES[DEFAULT_WINDOW_BACKBONE]["delayed"],
    "stim": WINDOW_BACKBONES[DEFAULT_WINDOW_BACKBONE]["stim"],
    "post": WINDOW_BACKBONES[DEFAULT_WINDOW_BACKBONE]["post"],
}
ACTIVE_WINDOW_BACKBONE = DEFAULT_WINDOW_BACKBONE
...
def _normalize_window(window: Sequence[float], name: str) -> Tuple[float, float]:
    if len(window) != 2:
        raise ValueError(f"{name} must contain exactly two floats")
    start = float(window[0])
    end = float(window[1])
    if not np.isfinite(start) or not np.isfinite(end):
        raise ValueError(f"{name} must contain finite values")
    if not end > start:
        raise ValueError(f"{name} must satisfy end > start")
    return (start, end)


def configure_windows(
    window_backbone: str | None = None,
    baseline_window: Sequence[float] | None = None,
    delayed_window: Sequence[float] | None = None,
    post_window: Sequence[float] | None = None,
) -> Dict[str, Tuple[float, float]]:
    backbone_name = DEFAULT_WINDOW_BACKBONE if window_backbone is None else str(window_backbone)
    if backbone_name not in WINDOW_BACKBONES:
        raise ValueError(f"Unknown window_backbone={backbone_name!r}; expected one of {sorted(WINDOW_BACKBONES)}")
    backbone = WINDOW_BACKBONES[backbone_name]
    baseline = _normalize_window(
        backbone["baseline"] if baseline_window is None else baseline_window,
        "baseline_window",
    )
    delayed = _normalize_window(
        backbone["delayed"] if delayed_window is None else delayed_window,
        "delayed_window",
    )
    post = _normalize_window(
        backbone["post"] if post_window is None else post_window,
        "post_window",
    )
    global WINDOWS, ACTIVE_WINDOW_BACKBONE
    WINDOWS = {
        "baseline": baseline,
        "delayed": delayed,
        "stim": delayed,
    ...
    ACTIVE_WINDOW_BACKBONE = backbone_name
    return get_canonical_windows()


def get_canonical_windows() -> Dict[str, Tuple[float, float]]:
    return {
        "baseline": WINDOWS["baseline"],
        "delayed": WINDOWS["delayed"],
        "post": WINDOWS["post"],
    }


def get_window_backbone_name() -> str:
    return str(ACTIVE_WINDOW_BACKBONE)


def get_window_backbone_metadata() -> Dict[str, Any]:
    backbone = WINDOW_BACKBONES[get_window_backbone_name()]
    return {
        "name": get_window_backbone_name(),
        "description": backbone["description"],
        "window_type": backbone["window_type"],
        "windows_seconds": {
            "baseline": WINDOWS["baseline"],
            "delayed": WINDOWS["delayed"],
            "stim": WINDOWS["stim"],
...
def _compute_window_means(
    ...
    delayed_offsets = (int(round(WINDOWS["delayed"][0] * frame_rate_hz)), int(round(WINDOWS["delayed"][1] * frame_rate_hz)))
    ...
    for trial_idx, stim_frame in enumerate(stim_frame_idx):
        ...
        delayed_start = stim_frame + delayed_offsets[0]
        delayed_end = stim_frame + delayed_offsets[1]
        ...
        if delayed_start < 0 or delayed_end > n_frames:
            quality_flags.append("delayed_out_of_bounds")
        ...
        if quality_flags:
            ...
            delayed_rows.append(np.full(n_neurons, np.nan, dtype=np.float32))
        ...
        delayed_rows.append(np.nanmean(traces[:, delayed_start:delayed_end], axis=1).astype(np.float32))
    ...
    delayed = np.vstack(delayed_rows).astype(np.float32)
    post = np.vstack(post_rows).astype(np.float32)
    diff = delayed - baseline
    summary_df = pd.DataFrame(records)
    return summary_df, {"baseline": baseline, "delayed": delayed, "stim": delayed, "post": post, "diff": diff}, warnings
...
def build_trials(
    ...
    delayed_window: Tuple[float, float] = WINDOWS["delayed"],
    post_window: Tuple[float, float] = WINDOWS["post"],
) -> Tuple[pd.DataFrame, Dict[str, Any], List[Dict[str, Any]]]:
    del baseline_window, delayed_window, post_window  # windows are kept centralized in WINDOWS
    root = Path(data_root).expanduser().resolve()
    ...
    for session in manifest["sessions"]:
        ...
        session_arrays: Dict[str, List[np.ndarray]] = {"baseline": [], "delayed": [], "stim": [], "post": [], "diff": []}
...
def select_neurons(
    ...
    for session_id, payload in neural_data.items():
        ...
        delayed_train = arrays.get("delayed", arrays["stim"])[train_idx]
        scores = _compute_responsiveness_scores(baseline_train, delayed_train)
        mean_effect = np.nanmean(delayed_train - baseline_train, axis=0)
...
def preprocess_to_copula_table(
    ...
    for session_id, selection in selections.items():
        ...
        combined_meta["window_type"] = str(get_window_backbone_metadata()["window_type"])
...
def write_outputs(
    ...
    write_json(
        ...
        {
            "window_backbone": get_window_backbone_name(),
            "window_backbone_metadata": get_window_backbone_metadata(),
            "windows_seconds": get_canonical_windows(),
            "windows_seconds_full": {
                "baseline": WINDOWS["baseline"],
                "delayed": WINDOWS["delayed"],
                "stim": WINDOWS["stim"],
                "post": WINDOWS["post"],
            },
            "window_aliases": {"stim": "delayed"} if np.allclose(WINDOWS["stim"], WINDOWS["delayed"]) else {},
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--window_backbone",
        choices=sorted(WINDOW_BACKBONES.keys()),
        default=DEFAULT_WINDOW_BACKBONE,
        help="Named Dalgleish response-window backbone.",
    )
    parser.add_argument(
        "--baseline_window",
        type=float,
        ...
        default=None,
        help="Optional baseline window override in seconds relative to stimulation onset.",
    )
    parser.add_argument(
        "--delayed_window",
        type=float,
        ...
        metavar=("START_SEC", "END_SEC"),
        default=None,
        help="Optional delayed or stimulation-aligned window override in seconds relative to stimulation onset.",
    )
    parser.add_argument(
        "--post_window",
        type=float,
        ...
        metavar=("START_SEC", "END_SEC"),
        default=None,
        help="Optional post-stimulation response window override in seconds relative to stimulation onset.",
    )
...
def main() -> None:
    ...
    active_windows = configure_windows(
        window_backbone=args.window_backbone,
        baseline_window=args.baseline_window,
        delayed_window=args.delayed_window,
        post_window=args.post_window,
    )
    LOGGER.info(
        "Using Dalgleish window backbone %s with windows (seconds): %s",
        get_window_backbone_name(),
        active_windows,
    )

...
`BenchmarkRunner` executes a Cartesian sweep over data scenarios, dimensions,
vine types, copula families, and optimization methods described by a
`BenchmarkConfig` dataclass, and writes the results to disk. This is the
lower-level Python API used by the `dvc-experiment` CLI entry point.

For the YAML-configured higher-level dispatcher, see
`dvc_package.experiments.experiment_framework.ExperimentRunner`.
"""
...
class BenchmarkRunner:
    ...
    def _create_visualizations(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Detailed publication plots are generated by optional downstream
            # workflows; this generic path records that the hook fired.
...
def run_experiment(config: Union[Dict[str, Any], BenchmarkConfig],
    ...
    Parameters
    ----------
    config : dict or BenchmarkConfig
    ...
    # Convert config to BenchmarkConfig if needed
    if isinstance(config, dict):
        if output_dir:
            config['output_dir'] = output_dir
        config['random_seed'] = seed
        exp_config = BenchmarkConfig(**config)
    else:
        exp_config = config
        if output_dir:
...
def _aggregate_multiple_runs(results_list: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        'summary': 'Multiple-run aggregation can be performed by downstream analysis scripts.',
